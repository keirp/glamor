...
def quick_affinity_code(n_parallel=None, use_gpu=True, contexts_per_gpu=1):
    ...
    if not (use_gpu or n_parallel):
        raise ValueError(
            "Either use_gpu must be True or n_parallel > 0 must be given.")
    import psutil
...
def encode_affinity(
        ...
        # Leave None in `run` script, but specified in `train` script.
        run_slot=None,
...
def get_n_run_slots(affinity_code):
    ...
    if aff.get("ass", 0) > 0:  # Asynchronous sample mode.
        total_gpu = aff.get("gpr", 1) + aff.get("sgr", 0) * \
            (1 - aff.get("oss", 0))
