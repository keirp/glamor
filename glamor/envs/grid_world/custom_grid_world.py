...
class CustomGridWorld(Env):
    ...
    def __init__(self, start_x, start_y, grid_file):
        ...
        self._observation_space = IntBox(
            low=0, high=255, shape=(2, self.w, self.h), dtype="uint8")
    ...
    def __init__(self, grid):
        ...
        for x in range(grid.shape[0]):
            for y in range(grid.shape[1]):
                if grid[x, y] == 0:
                    task_grid = np.zeros(
                        (2, grid.shape[0], grid.shape[1]), dtype=np.int)
                    task_grid[1, grid == 2] = 255
                    task_grid[0, x, y] = 255
                    info = EnvInfo(traj_done=False, labels=GridWorldLabels(
                        x=x, y=y), goal_labels=None)
