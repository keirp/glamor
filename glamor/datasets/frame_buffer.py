...
def frame_buffer_from_obs_buffer(obs_buffer):
	...
	if obs_buffer.current_buffer_size < obs_buffer.max_buffer_size:
		...
		fb.frame_buffer[obs_buffer.t + fb.n_frames - 1:] = obs_buffer.obs_buffer[obs_buffer.t:, -1]
		fb.frame_buffer[obs_buffer.t:obs_buffer.t + fb.n_frames - 1] = obs_buffer.obs_buffer[obs_buffer.t, :-1]
...
def obs_buffer_from_frame_buffer(frame_buffer):
	...
	ob.obs_buffer[:frame_buffer.current_buffer_size] = frame_buffer._get_obs(idxs)
...
class FrameBuffer:
	...
	def append_trajs(self, trajs):
		...
		for traj in trajs:
			...
			frame_idxs = (np.arange(self.t, self.t + T) + self.n_frames - 1) % (self.max_buffer_size + self.n_frames - 1)
			self.frame_buffer[frame_idxs] = stacked_obs[:, -1] # only most recent frames
	...
	def _get_obs(self, idxs):
		...
		lims = [np.arange(t - self.n_frames + 1, t + 1) % (self.max_buffer_size + self.n_frames - 1) for t in frame_idxs]
