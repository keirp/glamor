...
class EncoderLSTMModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.input_mix_nn = nn.Linear(
            self.state_size + n_actions, self.state_size)
        self.baseline_input_mix_nn = nn.Linear(
            self.obs_rep_size + n_actions, self.state_size)

        self.action_lstm = nn.LSTM(self.state_size, lstm_hidden_dim,
                                   batch_first=True, dropout=lstm_dropout_p, num_layers=lstm_layers)

        self.baseline_lstm = nn.LSTM(self.state_size, lstm_hidden_dim,
                                     batch_first=True, dropout=lstm_dropout_p, num_layers=lstm_layers)

        self.action_decoder = nn.Linear(lstm_hidden_dim, n_actions + 2)
        self.baseline_action_decoder = nn.Linear(
            lstm_hidden_dim, n_actions + 2)
    ...
    def get_state_mix(self, obs, tasks):
        ...
        task_rep = F.relu(self.task_encoder(
            tasks).view(-1, self.task_rep_size))
    ...
    def pred_actions(self, obs, tasks, actions):
        ...
        zero_action = torch.zeros(
            batch_size, 1, actions.shape[-1]).to(device=self.device)
        ...
        state_mix = state_mix.view(-1, 1,
                                   self.state_size).expand(-1, km1, self.state_size)
        ...
        # generate LSTM inputs
        obs_rep = obs_rep.view(-1, 1, self.obs_rep_size).expand(-1,
                                                                km1, self.obs_rep_size)
        ...
        p_actions_baseline = self.baseline_action_decoder(
            output).log_softmax(dim=-1)
    ...
    def pred_score_one_step(self, obs_rep, state_mix, prev_action, hiddens=None):
        ...
        info = {'p_actions': p_actions,
                'p_actions_baseline': p_actions_baseline}
    ...
    def policy(self, obs, tasks):
        ...
        zero_action = torch.zeros(
            batch_size, 1, self.n_actions).to(device=self.device)
        hidden = self.init_hidden_state(state_mix)
        ...
        p_actions = self.action_decoder(output).view(
            batch_size, -1)[:, :-2].softmax(dim=-1)
