...
class SamplingPlanner(BasePlanner):
    ...
    def sample_trajectory(self, state_mix, obs_rep, horizon, log=False):
        ...
        while t < horizon:
            prev_action_one_hot = torch.zeros(
                batch_size, self.n_actions + 2).to(device=self.device)
            if prev_action is not None:
                prev_action_one_hot = prev_action_one_hot.scatter_(
                    1, prev_action, 1)
            prev_action_one_hot = prev_action_one_hot.view(batch_size, 1, -1)

            score, hiddens, info = self.model.pred_score_one_step(
                obs_rep, state_mix, prev_action_one_hot, hiddens)
            ...
            if t == horizon - 1:
                prev_action = torch.ones_like(prev_action).to(
                    device=self.device) * self.end_token
            else:
            ...
            for key in info:
                if info[key].shape[1] > 1:
                    selected = info[key].gather(
                        1, prev_action).view(batch_size)
                else:
    ...
    def plan(self, obs, task, horizon):
        ...
        with torch.no_grad():
            ...
            traj, log_scores, ends, infos = self.sample_trajectory(
                state_mix, obs_rep, horizon)
