...
class LabelCompareEval(BaseEval):
	...
	def __init__(self, 
		...
		self.cutoffs = self._get_label_cutoffs(env_cls, horizon, n_bound_samples)
	...
	def eval(self, model):
		...
		avg_label_diffs['avg_pos_diff'] = self._get_euclidean_average(avg_label_diffs)
		...
		total_achieved = self._get_total_achieved(label_diffs, include=self.include_labels)
	...
	def compare_labels(self, traj_labels, task_labels):
		...
		for key in traj_labels:
			label_diffs[key] = abs(float(traj_labels[key]) - float(task_labels[key]))
