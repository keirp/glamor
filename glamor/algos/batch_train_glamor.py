...
class BatchTrainGLAMOR:
	...
	def __init__(self,
		...
		effective_r = float(self.n_batches * batch_size) / float(self.n_trajs * horizon)
		...
		self.ce_loss = torch.nn.NLLLoss(reduction='mean', ignore_index=self.n_actions + 1)
	...
	def model_fn(self, f_sample):
		...
		actions, baseline_actions = self.model.pred_actions(obs_0, obs_k, actions)
	...
	def train(self):
		...
		while self.total_steps < self.n_interactions:
			...
			self.model.eval()
			trajs = self.env_sampler.collect_trajectories(n_interactions=None, n_trajs=self.n_trajs)
